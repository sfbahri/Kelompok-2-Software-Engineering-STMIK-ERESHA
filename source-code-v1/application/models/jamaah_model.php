...
//        upload_usr_dok_kk   = '$username',
    ...
    function data_by_kloter_koper_dikirim($id_kloter){
        
        $sql = "SELECT koper,no_telp,nama,nama_paspor,alamat_pengiriman_koper,bayar_pernakpernik,bayar_batik FROM jamaah where id_kloter = '$id_kloter' and koper = 1 and is_deleted = 0 ORDER BY alamat_pengiriman_koper ASC";
        return $this->db->query($sql);
    ...
    function get_kloter(){
        ...
        $sql = "SELECT b.nama
                FROM jamaah as a
                INNER JOIN kloter as b ON a.`id_kloter` = b.id and b.`is_deleted` = 0
                where a.id = '$username' and a.is_deleted = 0";           
                
        return $this->db->query($sql);
         
    }
    
    function data_rekap_sipatuh($id_kloter){
        
        $sql = "SELECT *,DATE_FORMAT(tanggal_lahir, '%d-%m-%Y') tgl_lahir,CASE jenis_kelamin
                        WHEN 1 THEN 'L'
                        WHEN 2 THEN 'P'
                        ELSE NULL
                    END AS JENKEL,
                    CASE jenis_kelamin
                        ...
                        WHEN 2 THEN 'FEMALE'
                        ELSE NULL
                    END AS JENKEL_MANIFEST,
                    CASE status
                        WHEN 1 THEN 'Belum Kawin'
                        WHEN 2 THEN 'Kawin'
                        ELSE NULL
                    ...
                    DATE_FORMAT(paspor_tgl_keluar, '%d-%m-%Y') tgl_keluar_paspor,
                    DATE_FORMAT(paspor_tgl_exp, '%d-%m-%Y') tgl_exp_paspor,
                    DATE_FORMAT(tanggal_lahir, '%d %M %Y') tgl_lahir_manifest
            FROM jamaah where id_kloter = '$id_kloter' and is_deleted = 0 ORDER BY nama ASC";
        return $this->db->query($sql);
