...
class Produk_model extends CI_Model{	
    ...
    function data_list($kode_produk){
        $sql = "SELECT  a.kode as kode_produk,
                        ...
                        a.beli,
                        a.img_qrcode,
                        CONCAT(c.inisial, ' - ', c.nama, ' - ', a.size) AS nama_all
                FROM produk as a
                INNER JOIN outlet as b ON a.id_outlet = b.id and b.is_deleted = 0
                LEFT JOIN warna as c ON a.id_warna = c.id and c.is_deleted = 0
                WHERE a.is_deleted = 0 AND a.kode_produk_header = '$kode_produk'";
        return $this->db->query($sql);
    }
    
    function data_produk_publish(){
        $sql = "SELECT  a.nama AS nama_produk,
                ...
                LEFT JOIN produk_gambar AS b ON a.kode = b.kode_produk_header AND b.is_deleted = 0
                WHERE a.is_deleted = 0 
                ORDER BY a.id ASC";
        return $this->db->query($sql);
    }
    
    function data_list2($kode_produk){
        $sql = "SELECT  a.kode as kode_produk,
                        a.nama as nama_produk,
                        FORMAT(a.harga, 0) AS hargass,
                        b.nama as nama_outlet,
...
//        return $this->db->query($sql);
    ...
    function created_label($img_qrcode,$harga_jual,$kode_produksi,$kode_produksi_produk,$inisial,$nama,$data){
...
//            $this->db->query($sqlp_33);
    ...
    function upload_gambar_utama_media($data,$nama_file){
        
        $user_nik = $this->user_nik();
        
        $sql = "UPDATE produk_header SET img  = '$nama_file',
                                        updated_by = '$user_nik',
    ...
    function gambar_nama_update($data){
        
        $user = $this->session->userdata('sess_id_users');
        
        $sql = "UPDATE  produk_header SET     nama = '$data[nama_gambar]',
                                        updated_by       = '$user',
                                        updated_at       = NOW() WHERE kode = '$data[kode]'"; 

        $sql2 = "UPDATE  produk SET     nama = '$data[nama_gambar]',
                                        updated_by       = '$user',
                                        updated_at       = NOW() WHERE kode_produk_header = '$data[kode]'";
