...
FROM produksi AS a WHERE a.is_deleted = 0 ORDER BY a.id,a.created_at ASC";
    ...
    function data_detail($kode_produksi){
        ...
        //,jumlah as jumlah_finishing
        return $this->db->query($sql);
    }
    
    function data_detail_finishing($kode_produksi){
        
        $sql = "SELECT a.*,
                    DATE_FORMAT(a.tgl_mulai, '%d-%b-%Y') tgl_mulai,
                    DATE_FORMAT(c.tgl_serah_terima, '%d-%b-%Y') tgl_serahterima,
                    b.S1_inisial,
                    b.S2_inisial,
                    b.L1_inisial,
                    b.L2_inisial,
                    ...
                    b.L2_jumlah,
                    b.stok_at,
                    c.jumlah as jumlah_finishing,
                    c.*
                    FROM produksi AS a 
                    INNER JOIN produk_header AS b ON a.kode = b.kode_produksi AND b.is_deleted = 0
                    INNER JOIN produksi_finishing as C ON a.kode = c.kode_produksi AND c.is_deleted = 0
                    WHERE a.is_deleted = 0 and a.kode = '$kode_produksi'";
        
//        $sql = "SELECT *,
//                    DATE_FORMAT(tgl_mulai, '%d-%b-%Y') tgl_mulai,
//                    status 
//                FROM produksi WHERE is_deleted = 0 and kode = '$kode_produksi'";
        return $this->db->query($sql);
    }
...
// ---------------- END BIAYA UNTUK HISTORY KAS SEWING ------------------- //
        ...
        //INSERT KE PRODUK HEADER 
        $sql_22 = "INSERT INTO produk_header (kode,
                                            nama,
                                            kode_produksi,
                                            img_qrcode,
                                            created_by,
                                            created_at) 
                                    VALUES ('$data[kode_produk_header]',
                                        '$data[nama]',
                                        '$data[kode]',
                                        '$data[img_qrcode_produk_header]',
                                        '$user_nik',
                                        NOW())";
