...
class Penjualan_model extends CI_Model{	
    ...
    function detailtransaksi($no_nota){
        $sql = "SELECT a.*,
                    ...
                    format(a.uang_ongkir, 0) as uangongkir
...
//                    where a.no_nota = '$no_nota' and a.is_deleted = 0";
    ...
    function data_order_header_cetak($no_nota){
        $sql = "SELECT a.*,
                        ...
                        format(a.uang_bayar, 0) as uangbayar, 
                        format(a.uang_ongkir, 0) as uang_ongkir,
                        b.nama as jenis_pembayaran,
                        a.img_qrcode as qrcodes,
                        format((a.total + a.uang_ongkir), 0) as jumlah_tot,
                        a.total as tots
                FROM order_header as a
    ...
    function simpan_trans($data){
        ...
        $uang_ongkir = str_replace(",","",$data['uang_ongkir']);
    ...
    function update_trans($data){
        ...
        // start untuk update order sales
        $sql1 = "UPDATE order_header SET    no_order_sales         = '$data[no_order_sales]',
                                            updated_by           = '$user_nik',
                                            updated_at           = NOW() WHERE no_nota = '$data[no_nota]'";
        
        $this->db->query($sql1);
        
        $sql2 = "UPDATE order_sales SET     nonota         = '$data[no_nota]',
                                            updated_by           = '$user_nik',
    ...
    function transaksi_data(){
        ...
        if($user_nik == '00000001' || $user_nik == '00000002'){
            $sql = "SELECT a.*,
                ...
                where a.is_deleted = 0 ORDER by a.id_order DESC";
        return $this->db->query($sql);
    ...
    function data_order_request_barang($data){
        ...
        $sql = "SELECT  b.nama,
		        ...
		        INNER JOIN produk_gambar as c ON a.id_gambar = c.id and c.is_deleted = 0
                WHERE a.is_deleted = 0 and a.no_nota = '$data[nonota]'";
        /* and a.qty_s <> '' and a.qty_m <> '' and a.qty_l <> ''  */
        return $this->db->query($sql);
    ...
    function update_order_sales($data){
        ...
        $sql = "UPDATE order_sales SET     nonota         = '$data[no_nota]',
                                            updated_by           = '$user_nik',
                                            updated_at           = NOW() WHERE noos = '$data[no_order_sales]'";
    ...
    function retur_order_detail($data){
        
        $user_nik = $this->user_nik();
        
        //ambil data stok dari produksi ini by kode produksi
        $ss = "SELECT stok,harga FROM produk_header where kode = '$data[kode_produk_header]'";
        $d = $this->db->query($ss)->row_array();
        $stok_sekarang = $d['stok'];
        //ini stok sekarang dikurang satu, ( jumlah qty yang discan)
        $stok_akhir = $stok_sekarang + 1;
        
        $sqlxx = "DELETE FROM order_detail WHERE no_nota = '$data[no_nota]' and kode_produk_header = '$data[kode_produk_header]' and kode_produk = '$data[kode_produk]' ";
        $this->db->query($sqlxx);
        
        
        $sql = "INSERT INTO order_retur (no_nota,
                                    kode_produk_header,
                                    kode_produk,
                                    harga,
                                    created_by,
        ...
        $sqlc = "UPDATE produk SET beli = 0 WHERE kode = '$data[kode_produk]' and is_deleted = 0";
        $this->db->query($sqlc);
