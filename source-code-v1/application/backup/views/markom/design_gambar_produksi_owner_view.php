            ...
            success: function(result)
            {
                ...
                for ( var i=0 ; i<result.length ; i++ ) {
                   ...
                   var no = i+1;
                    ...
                    var link_lihat_gambar;
                    if(result[i].tanggal_terima_produksi == null){
                        status_bahanbaku = '<span style="color:red">Produksi Belum Terima Bahan Baku</span>';
                        link_lihat_gambar = "<a href='javascript:void(0)' onclick=\"getcontents('markom/design_gambar_produksi_upload_merah_owner','"+tokens+"','"+result[i].noso+"');\"><div class='btn btn-danger btn-sm' title='Lihat Data Gambar' ><i class='fa fa-file-image-o'></i></div></a>";
                    }else{
                        status_bahanbaku = '<span style="color:green">Produksi Sudah Terima Bahan Baku</span>';
                        link_lihat_gambar = "<a href='javascript:void(0)' onclick=\"getcontents('markom/design_gambar_produksi_upload_owner','"+tokens+"','"+result[i].noso+"');\"><div class='btn btn-info btn-sm' title='Lihat Data Gambar' ><i class='fa fa-file-image-o'></i></div></a>";
                    }
                   
                   
                    data.push([no,result[i].noso,result[i].nama,status_bahanbaku,link_lihat_gambar]);
