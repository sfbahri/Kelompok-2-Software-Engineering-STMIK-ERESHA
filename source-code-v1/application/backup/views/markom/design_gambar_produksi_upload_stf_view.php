...
$(document).ready(function(){
    ...
    list_gambar_produksi = function(){
        ...
        $.ajax({
        url       : base_url + 'markom/data_gambar_produksi_stf',
        type      : "post",
        dataType  : 'json',
                ...
                var html = '<div class="col-md-4" style="float:left;margin-bottom:30px"><div class="card" >'
                        html+='<figure class="overlay"><img class="card-img-top img-fluid" src="./uploads/produksi/'+res[i].gambar+'" alt="Image" class="img-fluid" ><figcaption class="overlay-body d-flex align-items-end justify-content-center">'
                                html+='<div class="img-option">'
                                  html+='<a href="javascript:void(0)" title="Upload Ulang Gambar" onclick=\"upload_ulang_img('+res[i].id+');\" class="img-option-link"><div><i class="fa fa-refresh" style="color:red"></i></div></a>'
                                  html+='<a href="javascript:void(0)" title="Approve | Kirim Gambar ke list gambar untuk diproduksi" onclick=\"img_approve('+res[i].id+');\" class="img-option-link"><div><i class="fa fa-check" style="color:green"></i></div></a>'
    ...
    upload_ulang_img = function(id){
        getpopup('markom/upload_ulang_img','<?php echo $this->session->userdata('sess_token');?>','popupedit',id);
    }
    ...
    img_approve= function(id){
        ...
        function(){
            ...
            $.ajax({
                url       : base_url + 'markom/gambar_approve_stf',
                type      : "post",
                dataType  : 'json',
                data      : {id : id,
                            <?php echo $this->security->get_csrf_token_name(); ?>:'<?php echo $this->security->get_csrf_hash(); ?>'
