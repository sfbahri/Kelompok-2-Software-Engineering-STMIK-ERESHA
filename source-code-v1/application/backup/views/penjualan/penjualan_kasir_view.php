<script>
    ...
    $("#btn_mulai").click(function(){
       ...
       // start : mulai transaksi
            ...
            var id_pelanggan        = $('#list_member').val();
            var nama_pelanggan      = $('#list_member :selected').text();
            var nohp_pelanggan      = $('#pelanggan_nohp').val();
            var email_pelanggan     = $('#pelanggan_email').val();
            var alamat_pelanggan    = $('#pelanggan_alamat').val();
        ...
        var no_nota = $("#no_nota").val();
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: base_url + 'order_sales/data_select',
                        data: {},
                        dataType  : 'json',
                        success: function (data) {
                            $('#list_order_sales').empty();
                            var $kategori = $('#list_order_sales');
                            $kategori.append('<option value=0>- Pilih No Order Sales -</option>');
                            for (var i = 0; i < data.length; i++) {
                                if(res.no_order_sales == data[i].noos){
                                    $kategori.append('<option value=' + data[i].noos + ' selected>'+ data[i].noos +' - '+ data[i].nama_pelanggan + '</option>'); 
                                }else{
                                     $kategori.append('<option value=' + data[i].noos + '>'+ data[i].noos +' - '+ data[i].nama_pelanggan + '</option>');
                                }
                               // $kategori.append('<option value=' + data[i].noos + '>'+ data[i].noos +' - '+ data[i].nama_pelanggan + '</option>');
                            }
                            $('#list_order_sales').trigger("chosen:updated");
                            $("#list_order_sales").chosen({width: "100%"});
                        }
                    });
                    /* select Kategori */
        ...
        $.ajax({ 
            ...
            success: function(result)
            {
                ...
                for ( var i=0 ; i<result.length ; i++ ) {
                    ...
                    data.push([no,result[i].nama_produks+'<span class="tx-11 d-block"><b>KD_PRODUK : '+result[i].kode_produk+'</b></span>',result[i].hargas,result[i].count_qty,result[i].count_harga,link_hapus]);
...
//                }
    ...
    $("#btn_update_order_sales").click(function(){
        
        var nonota              = $("#no_nota").val();
        
       $.ajax({
            url       : base_url + 'penjualan/update_order_sales',
            type      : "post",
            dataType  : 'json',
            data      : {no_order_sales  : $('#list_order_sales').val(),
                         no_nota         : nonota,
                        <?php echo $this->security->get_csrf_token_name(); ?>:'<?php echo $this->security->get_csrf_hash(); ?>'
                     },
            success: function (resp) {
                if(resp == true){
                    Command: toastr["success"]("No Order Sales Berhasil Diupdate", "Oke Berhasil");
                    data_order_sales();
                    get_detail_transaksi();
                }else{
                    Command: toastr["error"]("Transaksi Error, data tidak tersimpan !!", "Error");
            ...
            <div class="col-md-8">
                    ...
                    <hr>
                        ...
                        <div id="form_input_button" style="display:none">
                            ...
                            <button class="btn btn-info pd-y-13 pd-x-20 bd-0 mg-md-l-10 mg-t-10 mg-md-t-0 tx-uppercase tx-11 tx-spacing-2" id="btn_scan" style="cursor:pointer;margin-right: 3px" data-toggle="modal" data-target="#exampleModal2" ><i class="icon ion-ios-search"></i> Cari Manual</button>
                            <button class="btn btn-warning pd-y-13 pd-x-20 bd-0 mg-md-l-10 mg-t-10 mg-md-t-0 tx-uppercase tx-11 tx-spacing-2" id="btn_scan" style="cursor:pointer" data-toggle="modal" data-target="#exampleModal3"><i class="fa fa-pencil"></i> INPUT ORDER SALES</button>
                            </div>
                            <br>
                                ...
                                <div class="col-md-6">
                                    <div class="card">
                                        <div class="card-body">
                                            ...
                                            <input type="text" id="uang_ongkir" name="uang_ongkir" class="form-control maskmoney"/>
                                        </div><!-- card-body -->
                                    </div><!-- card -->
                                </div>
                            </div>
                            ...
                            <button class="btn btn-info pd-y-13 pd-x-20 bd-0 mg-md-l-10 mg-t-10 mg-md-t-0 tx-uppercase tx-11 tx-spacing-2" id="btn_cetak_dotmetrik" style="cursor:pointer;display:none"><i class="fa fa-print"></i> Cetak Dotmetrik</button>
...
<div class="modal fade" id="exampleModal3" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    ...
    <div class="modal-content">
        <div class="modal-header pd-y-20 pd-x-25">
            <h6 class="tx-14 mg-b-0 tx-uppercase tx-inverse tx-bold">ORDER SALES</h6>
            <button type="button" class="close btn-clear" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
        </div>
        <div class="modal-body">
            <label for="demo-vs-definput" class="control-label">No. Order Sales</label>
            <select class="form-control"  id="list_order_sales" name="list_order_sales"></select>
        </div>
        <div class="modal-footer">
            <button type="button" class="btn btn-info" id="btn_update_order_sales"><i class="fa fa-refresh"></i> Update</button>
