...
$(document).ready(function(){
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $('#tgl_mulai').val(response.sewing_tgl_mulai);
            $('#tgl_diambil').val(response.sewing_tgl_ambil);
            $('#biaya').val(response.sewing_biaya);
            $('#pic').val(response.sewing_pic);
            
            kas_sewing(response.sewing_id_kas);
            select_vendor(response.sewing_id_vendor);
            
            if(response.sewing_status == 2){
                $("#btn_simpan").hide();
                $("#btn_selesai_sewing").show();
                $("#base-tab2").show();
            }else if(response.sewing_status == 3){
                $("#btn_simpan").hide();
                $("#btn_selesai_sewing").show();
                $("#base-tab2").show();
            }else{
                $("#btn_simpan").show();
                $("#btn_selesai_sewing").hide();
                $("#base-tab2").hide();
            }
    ...
    kas_sewing = function(id){
        $.ajax({
            type: 'POST',
            url: base_url + 'kas/kas_sewing',
            data: {},
            dataType  : 'json',
            success: function (data) {
                $('#kas').empty();
                var $kategori = $('#kas');
                $kategori.append('<option value=0>- Pilih Kas Sewing -</option>');
        ...
        function(){
            ...
            $.ajax({
                ...
                success: function (response) {
                    if(response == true){
                            swal.close();
                            Command: toastr["success"]("Sewing Berhasil Disimpan", "Berhasil");
                            $("#btn_simpan").hide();
                            $("#base-tab2").show();
                            get_produksi_detail();
                    }else{
    ...
    $('#tgl_mulai,#tgl_diambil,#tgl_kembali').datepicker();
            ...
            $.ajax({
                ...
                success     : function(response){
                    if(response == true){
                        $('#'+id_modal).modal('hide');   
                        swal.close();
                        Command: toastr["success"]("Proses Sewing Selesai", "Berhasil");
                        getcontents('sewing','<?php echo $tokens;?>');
                    }else{
                        Command: toastr["error"]("Simpan error, data tidak berhasil disimpan", "Error");
                    } 
            },
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
        <div class="modal-header">
            <h4 class="modal-title">Detail Sewing</h4>
        </div>
        <div class="modal-body">
            ...
            <div class="tab-content px-1 pt-1">
              <div role="tabpanel" class="tab-pane active" id="tab1" aria-expanded="true" aria-labelledby="base-tab1">
                ...
                <div class="form-group">
                    <label for="demo-vs-definput" class="control-label">Kas Sewing</label>
              ...
              <div class="tab-pane" id="tab2" aria-labelledby="base-tab2">
                  <form id="form_input2"  method=POST enctype='multipart/form-data'>
                    ...
                    <div class="form-group">
                        <label for="timesheetinput3">Tanggal Kembali</label>
                        <div class="position-relative has-icon-left">
    ...
    <div class="modal-footer">
        ...
        <button type="button" class="btn btn-info" id="btn_selesai_sewing" style="display:none"><i class="la la-check-square-o"></i> Selesai & Kirim data Ke Finishing</button>
