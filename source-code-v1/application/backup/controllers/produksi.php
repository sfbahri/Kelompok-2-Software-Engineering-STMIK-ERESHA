...
class Produksi extends CI_Controller {
    ...
    public function data_detail_finishing(){
        $kode_produksi     = $this->input->post('kode_produksi',TRUE);
        $result = $this->produksi_model->data_detail_finishing($kode_produksi)->row_array();
        echo json_encode($result);
    }
    ...
    public function simpan(){
        ...
        //ini kode random untuk token
            $tokensss_produk = "";
            $codeAlphabetPrd.= "87326484602476248762342t48723487623472868";
            $codeAlphabetPrd.= "87987498222249833598969897985203254564555";
            $codeAlphabetPrd.= "0123456789";
            $maxprd = strlen($codeAlphabetPrd) - 1;
            for ($i=0; $i < 5; $i++) {
                $tokensss_produk .= $codeAlphabetPrd[mt_rand(0, $maxprd)];
            } 
            $today = date("Ymd");
            $data['kode_produk_header'] = $tokensss_produk.$today;
            $kode_produk_header = $tokensss_produk.$today;
        //ini kode random untuk token
...
//        
        ...
        //ini qrcode produk header

        $config2['cacheable']    = true; //boolean, the default is true
        $config2['cachedir']     = './uploads/'; //string, the default is application/cache/
        $config2['errorlog']     = './assets/'; //string, the default is application/logs/
        $config2['imagedir']     = './uploads/qrcode/'; //direktori penyimpanan qr code
        ...
        $image_name = $kode_produk_header.'.png'; //buat name dari qr code sesuai dengan nim

        $params2['data'] = $kode_produk_header; //data yang akan di jadikan QR CODE
        $params2['level'] = 'H'; //H=High
        $params2['size'] = 10;
        $params2['savename'] = FCPATH.$config2['imagedir'].$image_name; //simpan image QR CODE ke folder assets/images/
        $this->ciqrcode->generate($params2); // fungsi untuk generate QR CODE
        $data['img_qrcode_produk_header'] = $image_name;
        /*End Qr Code*/
