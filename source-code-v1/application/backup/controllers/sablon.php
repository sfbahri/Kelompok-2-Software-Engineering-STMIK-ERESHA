...
class Sablon extends CI_Controller {
    ...
    public function data_detail(){
        ...
        $result = $this->sablon_model->data_detail($kode_produksi)->row_array();
        echo json_encode($result);
    }
    ...
    public function update(){
        ...
        $data['tgl_mulai']                  = $this->input->post('tgl_mulai_sablon',TRUE);
        $data['tgl_kirim']                  = $this->input->post('tgl_kirim_sablon',TRUE);
        $data['vendor']                     = $this->input->post('vendor_sablon',TRUE);
        $data['biaya']                      = $this->input->post('biaya_sablon',TRUE);
        $data['jenis_barang']               = $this->input->post('jenis_barang_sablon',TRUE);
        $data['berat']                      = $this->input->post('berat_sablon',TRUE);
        $data['qty_awal']                   = $this->input->post('jumlah_awal_sablon',TRUE);
        $data['qty_akhir']                  = $this->input->post('jumlah_akhir_sablon',TRUE);
        $data['kode_produksi']              = $this->input->post('kode_produksi_sablon',TRUE);
        
        
        $data['c_gambar']                   = $this->input->post('c_gambar_sablon',TRUE);
        $date                               = date('dmY');
        $file_name                          = 'img_'.$date.time().$token;
        $nama_file;

        if($_FILES["c_gambar_sablon"]['name'] !=''){
        ...
        $this->upload->do_upload('c_gambar_sablon');
        ...
        /* ini ukuran 300x300 */
         ...
         $this->image_lib->clear();
            ...
            $nama_file = $this->input->post('c_gambar_sablon_asli',TRUE);
        }
